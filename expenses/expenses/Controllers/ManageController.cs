...
namespace expenses.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var sueldo = new ExpensesEF.Sueldo();
            string _codigo;

            _codigo = System.Threading.Thread.CurrentThread.CurrentCulture.ToString().Substring(0, 2);
            sueldo = context.Sueldo.Where(u => u.idUserGasto == userId).FirstOrDefault();
            ...
            var model = new IndexViewModel
            {
                ...
                VerTpcGastovsMesAnterior = context.AspNetUsers.Where(u => u.Id == userId).FirstOrDefault().VerTpcGastovsMesAnterior.GetValueOrDefault(),
                VerGastosTodosOSinRecurrentes = context.AspNetUsers.Where(u => u.Id == userId).FirstOrDefault().VerGastosSinOConRecurrentes.GetValueOrDefault(),
                _idSelectedGrupoGasto= context.AspNetUsers.Where(u => u.Id == userId).FirstOrDefault().idGrupoGastoPorDefecto.GetValueOrDefault(),
                _GrupoGastosTrad = GetListItemsGrupoGastos(context.GrupoGastoTextosTraduccion.Where(x => (x.GrupoGasto.Activo == true) && (x.idIdioma == context.Idiomas.Where(y => y.codigo == _codigo).FirstOrDefault().idIdioma)).ToList().OrderBy(x => x.Descripcion)),
                _idSelectedTipoPago = context.AspNetUsers.Where(u => u.Id == userId).FirstOrDefault().idTipoPagoPorDefecto.GetValueOrDefault(),
                _TiposPagoTrad = GetListItemsTiposPagos(context.TipoPagoTextosTraduccion.Where(x => (x.idIdioma == context.Idiomas.Where(y => y.codigo == _codigo).FirstOrDefault().idIdioma)).ToList().OrderBy(x => x.Descripcion)),
                DatosSueldos = (sueldo!=null?sueldo.SueldoBrutoAnual + "|" + sueldo.SueldoNetoEnero + "|" + sueldo.SueldoNetoFebrero + "|" + sueldo.SueldoNetoMarzo + "|" + sueldo.SueldoNetoAbril + "|" + sueldo.SueldoNetoMayo + "|" + sueldo.SueldoNetoJunio + "|" + sueldo.SueldoNetoJulio + "|" + sueldo.SueldoNetoAgosto + "|" + sueldo.SueldoNetoSeptiembre + "|" + sueldo.SueldoNetoOctubre + "|" + sueldo.SueldoNetoNoviembre + "|" + sueldo.SueldoNetoDiciembre:"")
            };

            //model.EnableColorGastoRecurrenteGrid = context.AspNetUsers.Where(u => u.UserName==userId).FirstOrDefault(). true;
            return View(model);
        }
        ...
        [HttpPost]
        [Authorize]
        public ActionResult UpdateUserOptions(IndexViewModel model, string submitButton)
        {
            switch (submitButton)
            {
                case "Update":
      ...
      private IEnumerable<SelectListItem> GetListItemsGrupoGastos(IEnumerable<ExpensesEF.GrupoGastoTextosTraduccion> elements)
      {
        ...
        private IEnumerable<SelectListItem> GetListItemsTiposPagos(IEnumerable<ExpensesEF.TipoPagoTextosTraduccion> elements)
        {
            // Create an empty list to hold result of the operation
            var selectList = new List<SelectListItem>();
